...
export default defineEventHandler(async (event) => {
    ...
    if(!query || query.login?.length === 0 || query.password?.length === 0) return {status: 400, body: "Un login et un mot de passe sont requis pour se connecter"}

    if(event.context.session.user) return {status: 400, body: "Vous êtes déjà connecté"}
