...
export default defineEventHandler(async (event) => {
    ...
    const sql = !query.sujet_id ? `SELECT * FROM messages` : `SELECT messages.id, messages.contenu, messages.date_crea, messages.date_modif, forum_id, users.id as user_id, users.login FROM messages 
